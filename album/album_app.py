...
def index():
    ...
    if current_user.is_authenticated:
        ...
        return render_template(
            "guest_home.html", user=current_user, albums=None, form=form
        )
...
@album_bp.route(
    "/<string:user_id>/albums/<string:album_name>",
    methods=[
        "DELETE",
    ],
)
...
def view_photo(user_id, album_name, photo_name):
    ...
    if current_user.is_authenticated and current_user.id == user_id:
        ...
        photo = album.photos.filter_by(
            name=photo_name, album_id=album.id
        ).first_or_404()
...
@album_bp.route(
    "/<string:user_id>/albums/<string:album_name>/<string:photo_name>",
    methods=[
        "DELETE",
    ],
)
...
@album_bp.route(
    "/<string:user_id>/albums/<string:album_name>/<string:photo_name>/comment",
    methods=[
        "POST",
    ],
)
def add_comment(user_id, album_name, photo_name):
    ...
    if current_user.is_authenticated:
        ...
        user.add_notification(
            notification_type="comment", type_id=comment.id, who_id=who_id
        )
        ...
        photo = album.photos.filter_by(name=photo_name).first_or_404()
        comment = Comment(
            display_name=f"anon", photo_id=photo.id, comment=json["comment"]
        )
        db.session.add(comment)
        db.session.commit()
        db.session.refresh(comment)
...
def update_profile():
    ...
    if form.validate():
        ...
        if request.files["photo"].filename != "":
            default_pro_pic_path = os.path.join(
                "album", "static", "users", str(current_user.id), "profileavatar.jpg"
            )
            if os.path.exists(default_pro_pic_path):
                os.remove(default_pro_pic_path)
            file_path = uploaded_images.save(
                request.files["photo"], f"{current_user.id}", name="profileavatar.jpg"
            )
        ...
        dict(
            users=[
                dict(
                    ...
                    url=url_for("album.view_albums", user_id=user.slugified_id()),
                )
                for user in users
            ]
        )
...
def get_notifications():
    notifications = current_user.notifications.filter(
        Notification.timestamp > current_user.notification_last_read
    ).all()
    current_user.notification_last_read = datetime.datetime.now()
    db.session.commit()
    return jsonify(
        [
            helpers.format_notification(notification, current_user)
